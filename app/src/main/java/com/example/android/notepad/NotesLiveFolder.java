...
//此活动创建一个活动文件夹意图并将其发送回HOME。HOME从Intent中的数据创建一个活动文件夹，并在HOME视图中显示其图标。当用户单击图标时，Home使用它从意图中获得的数据从内容提供者检索信息并将其显示在视图中。
//此活动的intent过滤器设置为ACTION_CREATE_LIVE_FOLDER, HOME将其发送给一个长按下并选择Live文件夹的响应。
