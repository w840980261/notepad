...
//此活动允许用户编辑笔记的标题。它显示一个浮动窗口
//包含EditText。

//注意:请注意此活动中的提供程序操作是在UI线程上进行的。
//这不是一个好的做法。这样做只是为了使代码更具可读性。真正的应用程序应该使用{@link android.content。AsyncQueryHandler}或{@link android.os。对象来异步地在单独的线程上执行操作。
